...
function useLayoutMigrationState() {
  const { targets } = useMigration();

  const [results, setResults] = useState<SerializedLayoutVariablesSuggestionsResults | null>(null);

  const [currentlyViewingEntryId, setCurrentlyViewingEntryId] = useState<ListEntry | null>(null);

  const hasAllItemsSelectedNewLayoutVariableId = useMemo(() => {
    // 로딩 중인 경우 done은 아님
  ...
  const showNextListItemWithoutSelectedLayoutVariable = useCallback(() => {
  ...
  const showNextListGroupWithoutSelectedLayoutVariable = useCallback(() => {
  ...
  const applyLayoutVariable = useCallback(
    ({
      oldValue,
      consumerNodeIds,
      variableId,
      ...
      emit<ApplyLayoutVariableEventHandler>("APPLY_LAYOUT_VARIABLE", {
        oldValue,
        consumerNodeIds,
        variableId,
      });
  ...
  const requestSuggestions = useCallback(() => {
    ...
    emit<RequestLayoutSuggestionsEventHandler>("REQUEST_LAYOUT_SUGGESTIONS", {
  ...
  useEffect(() => {
    on<SuggestLayoutVariablesEventHandler>("SUGGEST_LAYOUT_VARIABLES", ({ results }) => {
      setResults(results);
  ...
  return {
    results,
    hasAllItemsSelectedNewLayoutVariableId,
    progress,
    setResults,
    currentlyViewing,
    setCurrentlyViewingEntryId,
    showNextListItemWithoutSelectedLayoutVariable,
    showNextListGroupWithoutSelectedLayoutVariable,
    applyLayoutVariable,
    requestSuggestions,
  };
}

const LayoutMigrationStateContext = createContext<ReturnType<
  typeof useLayoutMigrationState
> | null>(null);

export function LayoutMigrationStateProvider({ children }: PropsWithChildren) {
  return (
    <LayoutMigrationStateContext.Provider value={useLayoutMigrationState()}>
      {children}
    </LayoutMigrationStateContext.Provider>
  );
}

export function useLayoutMigration() {
  const context = useContext(LayoutMigrationStateContext);

  if (!context)
    throw new Error("useLayoutMigration must be used within LayoutMigrationStateProvider");

  return context;
}

export function getOldValueId(
  value: SerializedLayoutVariablesSuggestionsResults[number]["oldValue"],
) {
  return `${value}`;
}
