...
import { useStrokeWeightAndCornerRadiusMigration } from "@/features/design-system/ui/stroke-weights-and-corner-radiuses/context";
import { LoadingIndicator } from "@/features/design-system/ui/components/loading-indicator";
import { Tooltip } from "@/shared/components/tooltip";

export function LayersWithStrokeWeightAndCornerRadiusList() {
  const { results, progress } = useStrokeWeightAndCornerRadiusMigration();
...
function LayerGroup({ groupId }: Pick<ListEntry, "groupId">) {
  const { results, currentlyViewing, setCurrentlyViewingEntryId } =
  ...
  return (
    <ListGroupButton
      ...
      title={getOldValueName(group.oldValue)}
    />
  );
}
...
function Layer({
  ...
  consumer: SerializedStrokeWeightAndCornerRadiusVariablesSuggestionsResults[number]["consumers"][number];
}) {
  const { setCurrentlyViewingEntryId, currentlyViewing } =
    useStrokeWeightAndCornerRadiusMigration();
