...
export function Result() {
  const { currentlyViewing } = useStrokeWeightAndCornerRadiusMigration();
...
function LayerResult() {
  const { currentlyViewing, applyStrokeWeightAndCornerRadiusVariable } =
  ...
  return (
    <RightPanel>
      <div className="flex flex-col gap-1 p-2 pb-1 w-full">
        ...
        <H2>
          <span className="font-bold">{oldValue.type}</span>
...
function VariableSuggestionButton({
  ...
  isSelected?: boolean;
  suggestion: SerializedStrokeWeightAndCornerRadiusVariablesSuggestionsResults[number]["suggestions"][number];
