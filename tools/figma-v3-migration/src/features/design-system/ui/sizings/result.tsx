...
import { useSizingMigration } from "@/features/design-system/ui/sizings/context";
import { Banner, IconWarning16 } from "@create-figma-plugin/ui";

export function Result() {
  const { currentlyViewing } = useSizingMigration();
...
function LayerResult() {
  const { currentlyViewing, applySizingVariable } = useSizingMigration();
      ...
      <div className="grow flex flex-col gap-2 w-full p-1.5 pt-0.5">
        ...
        {closestInstanceNode && (
          <Fragment>
            ...
            <Banner variant="warning" icon={<IconWarning16 />}>
              인스턴스 내부에 있는 레이어는 크기를 바꿀 수 없기 때문에 토큰을 연결할 수 없습니다.
              원본 컴포넌트를 수정해야 합니다.
            </Banner>
          </Fragment>
        )}
        {suggestions.map((suggestion) => (
          <VariableSuggestionButton
            ...
            isDisabled={closestInstanceNode !== null}
          />
        ))}
      </div>
    </RightPanel>
  ...
  return (
    <RightPanel>
      ...
      <div className="grow flex flex-col gap-2 w-full p-1.5 pt-0.5">
        ...
        {hasSelectedNodesWithinInstance && (
          <Fragment>
            <InstanceWarningBanner />
            <Banner variant="warning" icon={<IconWarning16 />}>
              선택한 레이어 중 인스턴스 내부에 있는 레이어가 있습니다. 인스턴스 내부에 있는 레이어는
              크기를 바꿀 수 없기 때문에 토큰을 연결할 수 없습니다. 원본 컴포넌트를 수정해야 합니다.
            </Banner>
          </Fragment>
        )}
        {suggestions.map((suggestion) => (
          <VariableSuggestionButton
            ...
            isDisabled={hasSelectedNodesWithinInstance}
          />
        ))}
      </div>
    </RightPanel>
...
function VariableSuggestionButton({
  ...
  isSelected?: boolean;
  suggestion: SerializedSizingVariablesSuggestionsResults[number]["suggestions"][number];
