...
import { type ListEntry, getOldValueId } from "@/features/design-system/ui/sizings/context";
...
export function LayersWithSizingList() {
  const { results, progress } = useSizingMigration();
...
function LayerGroup({ groupId }: Pick<ListEntry, "groupId">) {
  const { results, currentlyViewing, setCurrentlyViewingEntryId } = useSizingMigration();
...
function Layer({
  ...
  consumer: SerializedSizingVariablesSuggestionsResults[number]["consumers"][number];
}) {
  const { setCurrentlyViewingEntryId, currentlyViewing } = useSizingMigration();
