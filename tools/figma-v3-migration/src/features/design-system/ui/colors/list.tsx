      ...
      <Tooltip
        label={
          progress.left === 0
            ...
            : `${progress.left}개의 항목이 아직 V2 토큰을 사용 중이거나 Detach 상태입니다.`
        }
      >
        <Progress
...
function LayerGroup({ groupId }: Pick<ListEntry, "groupId">) {
  const { results, currentlyViewing, setCurrentlyViewingEntryId } = useColorMigration();
  ...
  return (
    <ListGroupButton
      ...
      {...(group.oldValue.type !== "uncheckable" && {
        startElement: (
          <ColorChip backgroundColor={`#${group.oldValue.hex}`} opacity={group.oldValue.opacity} />
        ),
      })}
    />
  );
