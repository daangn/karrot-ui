import { useProgress, useProgressCircle, type UseProgressProps } from "@seed-design/react-progress";
import {
  progressCircle,
...
export const ProgressCircle = React.forwardRef<SVGSVGElement, ProgressCircleProps>((props, ref) => {
  const { className, style, size = "40", tone = "neutral", ...otherProps } = props;
  const { value, percent, indeterminate, progressProps } = useProgress(otherProps);
  ...
  const ariaLabel = otherProps["aria-label"] || indeterminate ? "loading..." : `${percent} percent`;

  return (
    // biome-ignore lint/a11y/noSvgWithoutTitle: aria-label is always provided
    <svg
      ref={ref}
      {...otherProps}
      {...progressProps}
